...
def generate_trials(size, mem_gap):
    ...
    var_delay_length = 100    # change for a variable length stimulus
    ...
    for ii in np.arange(sample_size): 
        x_train_[ii, first_in+ var_delay[ii]:first_in + var_delay[ii]+ stim_dur, 0]   = xor_seed_A[trial_types[ii], 0]+  stim_noise * np.random.randn(stim_dur)
        x_train[ii, first_in:first_in + stim_dur, 0]    = signal.convolve(x_train_[ii, first_in:first_in + stim_dur, 0], win, mode='same') / sum(win) 
      
        y_train[ii, out_t:, 0]  = np.sign(scipy.integrate.simps(x_train[ii, first_in+ var_delay[ii]:first_in + stim_dur+ var_delay[ii], 0], x=None, dx=1, axis=-1, even='avg')) 
