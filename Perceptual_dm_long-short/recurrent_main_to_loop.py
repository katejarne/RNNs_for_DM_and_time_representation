...
from keras.layers import Input, Dense,  SimpleRNN, Activation, Dropout #TimeDistributed,
...
def DM_fun(t,N_rec,base,base_plot):
    ...
    model = Sequential()
    model.add(SimpleRNN(units=N_rec,return_sequences=True, input_shape=(None, 2), kernel_constraint=None, kernel_initializer='glorot_uniform',recurrent_initializer= "orthogonal",recurrent_constraint=None, use_bias=False))           #,use_bias=True,bias_initializer='zeros')) #defaults for the recurrent model!
    model.add(Dense(units=1,input_dim=N_rec))#NonNegLast_input()
    
    model.save(base+'/'+base_plot[-4]+base_plot[-3]+'_00_initial.hdf5')
    
    # Model Compiling:
    ADAM           = optimizers.Adam(learning_rate=0.0001, beta_1=0.9, beta_2=0.999,epsilon=1e-08, decay=0.0001, clipnorm=1.0)
