...
#def pepe_(shape, dtype=None):
#    shape = keras.initializers.RandomNormal(mean=0.0, stddev=np.sqrt(float(1)/float((N_rec))), seed=None)
#    return K.variable( shape, dtype=dtype)
...
class NonNegLast(Constraint):
    def __call__(self, w):
        first_cols= w[:,0:int(N_rec/3)]*K.cast(K.less_equal(w[:,0:int(N_rec/3)], 0.0), K.floatx())
        last_cols= w[:,int(N_rec/3):int(N_rec)]*K.cast(K.greater_equal(w[:,int(N_rec/3):int(N_rec)], 0.0), K.floatx())                                
